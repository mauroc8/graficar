...
function leer(cadena, índice) {
	...
	var cursor = 0,
		...
		operador = function(buscar) {
			...
			if(buscar ? cadena[cursor] === buscar : operadores.includes(cadena[cursor])) {
		...
		expresión = function() {
			...
			if(cadena[cursor] === "|") {
				error(!cadena.includes('|', cursor + 1), "Función de valor absoluto sin cerrar.");
			...
			if(cadena[cursor].match(/[a-z]/)) {
				error(!VARIABLES_PERMITIDAS.includes(cadena[cursor]),
							...
							if(elemento.tipo === "función" &&
							   ...
							   precede.cadena.includes(elemento.función)) {
								...
								if(precede.binario) {
									...
									subexpr = { tipo: "expresión", expresión: [cabeza, elemento, cola] };
...
var evaluar = function() {
	...
	var unarios = { // se refiere a unarios
		"sen": Math.sin,
		"cos": Math.cos,
		"tan": Math.tan,
		"abs": Math.abs,
